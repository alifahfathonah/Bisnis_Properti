...
$bagianWhere="";
	...
	//echo "SELECT * From properti WHERE ".$bagianWhere;
			$query=mysqli_query($con,"SELECT * From properti WHERE ".$bagianWhere." LIMIT $offset, $dataPerPage");
							while ($hasil3=mysqli_fetch_array($query)){
								$id_owner=$hasil3['id_owner'];
								$id_properti=$hasil3['id_properti'];
								$id_fasilitas=$hasil3['id_fasilitas'];
								...
								$query2=mysqli_query($con,"SELECT * From fasilitas where id_fasilitas='$id_fasilitas'");					
				...
				<div class="light">
						<div class="paginate wrapper"><!-- The "wrapper" is just a comestic addition. You don't need this for the pagination to work. -->
							...
							include "admin/koneksi.php";
								$hasilko  = mysqli_query($con,"SELECT COUNT(*) AS jumData FROM properti WHERE ".$bagianWhere);
								$data     = mysqli_fetch_array($hasilko);
								$jumData = $data['jumData'];
								$jumPage = ceil($jumData/$dataPerPage);
								if ($noPage > 1)
								{
									echo  "<li><a href='".$_SERVER['PHP_SELF']."?".$cat."&page=".($noPage-1)."'>&lang;</a></li>";
								}
								$showPage=9;
									...
									if ((($page >= $noPage - 3) && ($page <= $noPage + 3)) || ($page == 1) || ($page == $jumPage))
										{
											...
											else echo " <li><a href='".$_SERVER['PHP_SELF']."?".$cat."&page=".$page."'>".$page."</a></li>";
										$showPage = $page;
										}
								}
								if ($noPage < $jumPage) echo "<li><a href='".$_SERVER['PHP_SELF']."?".$cat."&page=".($noPage+1)."'>&rang;</a></li>";
								?>
							</ul>
						</div>
					</div><!--Pagination-->
